...
fulldata <- read_excel("MassSpec2.xlsx", sheet = "Fulldata")
colnames(fulldata) <- c("Condition", "Sample", "Trp")
...
ggplot(summary_stats, aes(x = reorder(Group, -Mean), y = Mean, fill = Group)) +
  ...
  labs(
    title = "Mean Trp by Strain with Standard Error",
    x = "Strain",
    y = "Trp Intensity"
  ) +
  theme_minimal() +
...
ref_vals_log <- log_data %>%
  group_by(Group) %>%
  summarise(Mean = mean(Trp), .groups = "drop") %>%
...
log_data <- log_data %>%
  ...
  filter(!is.na(Reference) & Reference > 0 & Trp > 0) %>%
  mutate(Log2FC = log2(Trp / Reference))
...
paired_data <- inner_join(stat_data, log_data, by = "Strain", suffix = c("_S", "_L")) %>%
  mutate(Log2FC_S_vs_L = abs(log2(Trp_S / Trp_L)))  # Or just log2(...) for signed
